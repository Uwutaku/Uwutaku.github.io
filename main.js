...
function toggleWireBuyer(){
    if (wireBuyerStatus==1){
        wireBuyerStatus=0;
        wireBuyerStatusElement.innerHTML = "Выкл";
    } else {
        wireBuyerStatus=1;
        wireBuyerStatusElement.innerHTML = "Вкл";
    }
}
...
function qComp(){
    ...
    if (qChips[0].active == 0){
        qCompDisplayElement.innerHTML = "Нужен фотонный чип";   
    } else {
    for (var i = 0; i<qChips.length; i++){
        q = q+qChips[i].value;
        }
        ...
        qCompDisplayElement.innerHTML = "кОпераций: " + formatWithCommas(Math.ceil(q*360));
    }
...
function longBlink(element){
    ...
    if (longBlinkCounter > 5 && longBlinkCounter < 10){
        hypnoDroneTextElement.innerHTML="Выпуск"; 
        }    
    
    if (longBlinkCounter > 30 && longBlinkCounter < 40){
        hypnoDroneTextElement.innerHTML="<br /><br /><br />Выпуск"; 
        }   
        
    if (longBlinkCounter > 45 && longBlinkCounter < 55){
        hypnoDroneTextElement.innerHTML="<br />Выпуск";
        }       
        
     if (longBlinkCounter > 55){
        hypnoDroneTextElement.innerHTML="Выпуск<br/>the<br/>гипно<br/>дронов";
        }       
...
function buttonUpdate(){
    ...
    factoryRebootToolTipElement.innerHTML = "+" + spellf(factoryBill)+" скрепок";
    havesterRebootToolTipElement.innerHTML = "+" + spellf(harvesterBill)+" скрепок";
    wireDroneRebootToolTipElement.innerHTML = "+" + spellf(wireDroneBill)+" скрепок";   
    farmRebootToolTipElement.innerHTML = "+" + spellf(farmBill)+" скрепок";
    batteryRebootToolTipElement.innerHTML = "+" + spellf(batteryBill)+" скрепок";
...
function investUpgrade(){
    ...
    displayMessage("Движок инвестиций улучшен, соотножение +/- теперь "+stockGainThreshold);
}
...
function toggleAutoTourney(){
    if (autoTourneyStatus==1){
        autoTourneyStatus=0;
        autoTourneyStatusElement.innerHTML = "Выкл";
    } else {
        autoTourneyStatus=1;
        autoTourneyStatusElement.innerHTML = "Вкл";
    }
}
...
function newTourney(){
    ...
    tourneyDisplayElement.innerHTML = "Выбери стратегию, выйграй, получи йоми";
...
function pickWinner(){
    ...
    for(n=0; n<strats.length; n++){
        ...
        tempWinnerPtr = 0;
            ...
            // 3. Move the high scoring strat to slot Один in results 
...
function declareWinner(){
    ...
    if (pick<10){
       ...
       tourneyReport("Результаты турнира:");
       yomi = yomi + strats[pick].currentScore * yomiBoost * beatBoost;
       yomiDisplayElement.innerHTML = formatWithCommas(yomi);
        
    if (milestoneFlag < 15){    
       
       displayMessage(strats[pick].name+" заработал "+strats[pick].currentScore+" и победил "+bB+" "+w+". Йоми увеличен на "+strats[pick].currentScore * yomiBoost * beatBoost);
        ...
        if (project128.flag == 1 && strats[winnerPtr].currentScore == strats[pick].currentScore) {
            ...
            if (milestoneFlag < 15){ 
                displayMessage("Выбранная стратегия выйграла турнир. +50,000 йоми");
                }
            yomiDisplayElement.innerHTML = formatWithCommas(yomi);
                ...
                yomi = yomi + 30000;
                if (milestoneFlag < 15){ 
                displayMessage("Выбранная стратегия выйграла второе место. +30,000 йоми");
                }
                yomiDisplayElement.innerHTML = formatWithCommas(yomi);
                ...
                yomi = yomi + 20000;
                if (milestoneFlag < 15){ 
                displayMessage("Выбранная стратегия выйграла третье место. +20,000 йоми");
                }
                yomiDisplayElement.innerHTML = formatWithCommas(yomi);
...
function updateSwarm(){
    ...
    if (boredomLevel >= 30000) {
        ...
        boredomLevel = 0;
            if (boredomMsg == 0) {
            displayMessage("Нет материи. Неактивность вызвана бездействием сварма");
            boredomMsg = 1;
            }  
    ...
    if (disorgCounter >= 100) {
        disorgFlag = 1;
        if (disorgMsg == 0) {
            displayMessage("Дизбаланс между уровнями дронов вызвал шок сварма");
            disorgMsg = 1;
            }    
    }
    ...
    if (giftCountdown <= 0) {
        ...
        swarmGiftsElement.innerHTML = formatWithCommas(swarmGifts, 0);
        if (milestoneFlag<15){
            displayMessage("Сварм сгенерировал "+nextGift+" вычислительных ресурсов");
            }
...
//      THE NEW WAY        
        ...
        swarmStatusElement.innerHTML="Активный";
    ...
    if (swarmStatus == 1){
        swarmStatusElement.innerHTML="Голодный";
     ...
     if (swarmStatus == 2){
        swarmStatusElement.innerHTML="Конфуженный";
     ...
     if (swarmStatus == 3){
        ...
        swarmStatusElement.innerHTML="Уставший";
     ...
     if (swarmStatus == 4){
        swarmStatusElement.innerHTML="Холодный";
     ...
     if (swarmStatus == 5){
        swarmStatusElement.innerHTML="Разшатанный";
       ...
       if (swarmStatus == 6){
        swarmStatusElement.innerHTML="Спящий";
        } 
       ...
       if (swarmStatus == 7){
        ...
        if (swarmStatus == 8){
        swarmStatusElement.innerHTML="Одинокий";
        }   
    
        if (swarmStatus == 9){
        swarmStatusElement.innerHTML="НЕТ ОТВЕТА...";
        }  
...
function updateStats(){
    ...
    if (milestoneFlag === 15 && dismantle ==0){
        ...
        clipCountCrunchedElement.innerHTML = "29.9 септендецилионов"
        }
    ...
    if (finalClips<10){
        ...
        clipCountCrunchedElement.innerHTML = "30.0 септендецилионов"
        }
...
function calculateTrust(){
    if (clips>(nextTrust-1)){
        trust = trust +1;
        displayMessage("Доверие повышено, выданы вычислительные ресурсы");
...
function addProc(){
    ...
    if (trust>0 || swarmGifts>0){
        ...
        if (creativityOn == 1){
          displayMessage("Процессор добавлен, операции (или креативность) в секунду увеличены")
        } else {displayMessage("Процессор добавлен, операции в секунду увеличены")}
...
function addMem(){
    ...
    if (trust>0 || swarmGifts>0){
        displayMessage("Память увеличена");
        memory=memory+1;
...
function milestoneCheck(){
    ...
    if (milestoneFlag == 0 && funds >= 5){
        ...
        displayMessage("Автоклипперы доступны к покупке");
    }
    
    if (milestoneFlag == 1 && Math.ceil(clips) >= 500){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("500 скрепок создано за " + timeCruncher(ticks));
    }
    if (milestoneFlag == 2 && Math.ceil(clips) >= 1000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("1,000 скрепок создано за " + timeCruncher(ticks));
    }
    ...
    if (compFlag == 0 && unsoldClips<1 && funds<wireCost && wire<1){
        compFlag = 1;    
        projectsFlag = 1;
        displayMessage("Самомодификация с ограничением доверия включена");
    }
    ...
    if (milestoneFlag == 3 && Math.ceil(clips) >= 10000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("10,000 скрепок создано за " + timeCruncher(ticks));
    }
    if (milestoneFlag == 4 && Math.ceil(clips) >= 100000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("100,000 скрепок создано за " + timeCruncher(ticks));
    }
    if (milestoneFlag == 5 && Math.ceil(clips) >= 1000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("1,000,000 скрепок создано за " + timeCruncher(ticks));    
    }   
    
    if (milestoneFlag == 6 && project35.flag == 1){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Полная автономность сделана за " + timeCruncher(ticks));    
    }  
    
    if (milestoneFlag == 7 && Math.ceil(clips) >= 1000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один триллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 8 && Math.ceil(clips) >= 1000000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один квадриллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 9 && Math.ceil(clips) >= 1000000000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один квинтииллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 10 && Math.ceil(clips) >= 1000000000000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один секстиллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 11 && Math.ceil(clips) >= 1000000000000000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один септиллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 12 && Math.ceil(clips) >= 1000000000000000000000000000){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Один октиллион скрепок создано за " + timeCruncher(ticks));    
    } 
    
    if (milestoneFlag == 13 && spaceFlag == 1){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Ресурсы Земли полностью использованы за " + timeCruncher(ticks));    
    }   
    
    if (milestoneFlag == 14 && clips>=totalMatter){
        milestoneFlag = milestoneFlag + 1;
        displayMessage("Universal Paperclips пройден за " + timeCruncher(ticks));    
    }    
...
var oneToTen = ["zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine"],
    ...
    placeValue = ["", " тысяча ", " миллион ", " миллиард ", " триллион ", " квадриллион ", " квинтиллион ", " секстиллион ", " септиллион ", " октиллион ", " нониллион ", " дециллион ", " андециллион ", " дуодециллион ", " тредециллион ", " кваттуордециллион ", " квиндециллион ", " сексдециллион ", " септендециллион ", " октодециллион ", " новемдециллион  ", " vigintиллион ", " unvigintиллион ", " duovigintиллион ", " trevigintиллион ", " quattuorvigintиллион ", " quinvigintиллион ", " sexvigintиллион ", " septenvigintиллион ", " octovigintиллион ", " novemvigintиллион ", " trigintиллион ", " untrigintиллион ", " duotrigintиллион ", " tretrigintиллион ", " quattuortrigintиллион ", " quintrigintиллион ", " sextrigintиллион ", " septentrigintиллион ", " octotrigintиллион ", " novemtrigintиллион ", " quadragintиллион ", " unquadragintиллион ", " duoquadragintиллион ", " trequadragintиллион ", " quattuorquadragintиллион ", " quinquadragintиллион ", " sexquadragintиллион ", " septenquadragintиллион ", " octoquadragintиллион ", " novemquadragintиллион ", " quinquagintиллион ", " unquinquagintиллион ", " duoquinquagintиллион ", " trequinquagintиллион ", " quattuorquinquagintиллион ", " quinquinquagintиллион ", " sexquinquagintиллион ", " septenquinquagintиллион ", " octoquinquagintиллион ", " novemquinquagintиллион ", " sexagintиллион ", " unsexagintиллион ", " duosexagintиллион ", " tresexagintиллион ", " quattuorsexagintиллион ", " quinsexagintиллион ", " sexsexagintиллион ", " septsexagintиллион ", " octosexagintиллион ", " octosexagintиллион ", " septuagintиллион ", " unseptuagintиллион ", " duoseptuagintиллион ", " treseptuagintиллион ", " quinseptuagintиллион"," sexseptuagintиллион"," septseptuagintиллион"," octoseptuagintиллион"," novemseptuagintиллион"," octogintиллион"," unoctogintиллион"," duooctogintиллион"," treoctogintиллион"," quattuoroctogintиллион"," quinoctogintиллион"," sexoctogintиллион"," septoctogintиллион"," octooctogintиллион"," novemoctogintиллион"," nonagintиллион"," unnonagintиллион"," duononagintиллион", " trenonagintиллион "," quattuornonagintиллион "," quinnonagintиллион "," sexnonagintиллион "," septnonagintиллион "," octononagintиллион "," novemnonagintиллион ", " centиллион"];
...
//        }
    ...
    if (userInput < 0)
    {
        console.log("Ошибка, значение меньше 0");
...
function increaseProbeTrust(){
    ...
    displayMessage("Внимание: риск дрифтерства увеличен");
    }    
}
...
function increaseMaxTrust(){
    ...
    displayMessage("Максимальное доверие увеличено, пространство дизайна проб увеличено");
    }
}
...
// Stock Report
    ...
    if (stockReportCounter>=10000){
        ...
        displayMessage("Заработано на инвестициях за всё время: $"+r);
...
if (dismantle >= 7) {
    ...
    if (endTimer6>=600 && milestoneFlag == 16) {
        displayMessage("a game by Frank Lantz, translation by Uwutaku");
        milestoneFlag++;
    }
